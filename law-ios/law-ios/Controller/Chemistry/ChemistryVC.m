...
-(void)finishSelectAction{
    ...
    if (self.section == 0) {
        ...
        NSDictionary *dickeyvelue = self.arrKeyValue2[self.row];
        self.arrKeyValue2[self.row] =@{dickeyvelue.allKeys[0]:dic.allValues[0]};
    }
}
...
-(void)selectAction{
    ...
    NSMutableArray *arrMuKeyValue1 = [NSMutableArray array];
    NSMutableArray *arrMuKeyValue2 = [NSMutableArray array];
    for (NSDictionary *dic in self.arrKeyValue1) {
        if (![dic.allValues[0]isEqualToString:@"无"]) {
            [arrMuKeyValue1 addObject:dic];
        }
    }
    for (NSDictionary *dic in self.arrKeyValue2) {
        if (![dic.allValues[0]isEqualToString:@"无"]) {
            [arrMuKeyValue2 addObject:dic];
        }
    }
    vc.arrKeyValue1 = arrMuKeyValue1;
    vc.arrKeyValue2 = arrMuKeyValue2;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSMutableAttributedString * mStr = [[NSMutableAttributedString alloc]initWithString:[NSString stringWithFormat:@"%@",model.nameCn]];
    NSRange range = [mStr.string rangeOfString:self.search.text];
    if (range.length > 0) {
