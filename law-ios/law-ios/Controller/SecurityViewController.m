...
- (void)viewDidLoad {
    ...
    self.headIgv.frame = CGRectMake(0, self.cycleScrollView.bottom, WIDTH_, 30);
    ...
    UIView * line;
    for (int i = 0; i < 2; i ++) {
        btn = [[UIButton alloc]initWithFrame:CGRectMake(0, self.headIgv.bottom+ 80 *i, WIDTH_, 80)];
        ...
        igv1.image = [UIImage imageNamed:@"Back Chevron Copy 4"];
        [btn addSubview:igv1];
...
-(UIImageView *)headIgv {
    if (!_headIgv) {
        ...
        igv.image = [UIImage getImageWithColor:[UIColor yellowColor] andSize:CGSizeMake(WIDTH_, 1)];
